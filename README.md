...
| `exp3_noise_robustness.py`          | NEW — noise sweep (σ = 0…10 %), mean ± std of identified α.             |
| `exp4_sensitivity_analysis.py`      | NEW — sweeps over depth, width, N_col, Δt.                              |
| `exp5_activation_ablation.py`       | NEW — sin vs tanh/GELU/SiLU/softplus/ReLU.                              |
| `exp6_dt_convergence.py`            | NEW — verifies observed O(Δt²) temporal convergence order.              |
| `exp7_cnoidal_wave.py`              | NEW — additional example: periodic cnoidal wave.                        |
| `exp8_mkdv_extension.py`            | NEW — extension to the modified KdV equation (cubic nonlinearity).      |
| `exp9_inverse_two_params.py`        | NEW — jointly identify λ₁ *and* α from the same data.                   |
| `exp10_conservation_laws.py`        | NEW — mass / momentum / Hamiltonian preservation diagnostic.            |
| `run_all.py`                        | Runs every experiment in order.                                             |

## Which figures / tables go where in the manuscript?

Suggested placement to strengthen the paper for Applied Soft Computing /
...
* § New — Additional worked examples (cnoidal, mKdV) —
  from `exp7_cnoidal` and `exp8_mkdv`.
* § New — Simultaneous identification of two parameters —
  from `exp9_two_params` — a strictly harder inverse benchmark.
* § New — Conservation-law preservation —
...
* Double precision (`torch.set_default_dtype(torch.float64)`) is enforced
  everywhere — third-order derivatives are ill-conditioned in float32.
* Gradient clipping (max_norm = 1.0) is applied in every *inverse* script to
  stabilise the joint identification of network weights and physical
  parameters.

## Quick smoke test

Reduce `EPOCHS` in each script (e.g. to 500) for a 30-second sanity check.

...
def u_cnoidal(x, t, m: float = 0.9, k: float = 1.0):
    """
    Cnoidal travelling wave of the KdV equation  u_t - 6 u u_x + u_xxx = 0.

    A convenient closed-form periodic solution:
        u(x, t) = -2 k^2 m * cn^2( k(x - c t), m )
    with  c = 4 k^2 (2 m - 1).

    Implemented in NumPy (scipy.special) then wrapped to a torch tensor - only
    used for exact-vs-predicted plotting so gradient support is unnecessary.
    """
    from scipy.special import ellipj
    xnp = x.detach().cpu().numpy() if isinstance(x, torch.Tensor) else np.asarray(x)
    tnp = t.detach().cpu().numpy() if isinstance(t, torch.Tensor) else np.asarray(t)
    c = 4.0 * k ** 2 * (2.0 * m - 1.0)
    arg = k * (xnp - c * tnp)
    cn, _, _, _ = ellipj(arg, m)
    u = -2.0 * (k ** 2) * m * cn ** 2
    if isinstance(x, torch.Tensor):
        return torch.tensor(u, dtype=torch.get_default_dtype(), device=x.device)
    return u
